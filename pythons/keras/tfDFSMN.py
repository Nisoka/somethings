...
class DFSMN(object):
    ...
    def __call__(self, skip_con, input_data):
        ...
        for step in range(num_steps):
            ...
            # c0 - cn  is the lookahead memory weight
            # shuold be tf.maximum(0, tf.minimum(self._memory_size_right, num_steps - step - self.memory_size_right -1))
            # 1 the num_steps is big an step is small, so the right memory should be the right memeory size
            # 2 the step is big, so will left  only the num_steps - step - self.memory_size_right -1
            # 3 if step is num_steps, so right_num is just 0
            right_num = tf.maximum(0, num_steps - step - self._memory_size_right - 1)

            # why do it as a backdirection???
        ...
        p_hat_skip = skip_connection_func(skip_con)
        p_hatt = tf.matmul([memory_matrix] * batch_size, input_data)
        p_hatt = p_hat_skip + p_hatt
